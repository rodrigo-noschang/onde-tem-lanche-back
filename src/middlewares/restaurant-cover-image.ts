...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        const url = RESTAURANT_COVER_IMAGE_BASE_URL;
        cb(null, url)
    },
    filename: function (req, file, cb) {
        ...
        const fileName = `cover-${namePrefix}-${restaurantId}.${fileExtension}`;

        cb(null, fileName);
    }
})

export const uploadRestaurantCoverImageMiddleware = multer({
    storage,
