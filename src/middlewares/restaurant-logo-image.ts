...
import { RESTAURANT_LOGO_IMAGE_BASE_URL } from '../static';

const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        const url = RESTAURANT_LOGO_IMAGE_BASE_URL;
        cb(null, url)
    },
    filename: function (req, file, cb) {
        ...
        const fileName = `logo-${namePrefix}-${restaurantId}.${fileExtension}`;

        cb(null, fileName);
    }
})

export const uploadRestaurantLogoImageMiddleware = multer({
    storage,
